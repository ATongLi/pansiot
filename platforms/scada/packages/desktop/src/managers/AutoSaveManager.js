...
class AutoSaveManager {
    constructor(mainWindow, config = {}) {
        ...
        this.autoSaveDir = path.join(electron_1.app.getPath('userData'), 'autosave');
        this.initialize();
     ...
     * 设置当前项目
     */
    setCurrentProject(projectPath, projectData) {
     ...
     * 读取自动保存文件
     */
    async readAutoSaveFile(filePath) {
        try {
     ...
     * 恢复自动保存文件
     */
    async restoreAutoSave(filePath) {
        try {
    ...
    async clearAllAutoSaves() {
        try {
            const files = await fs.readdir(this.autoSaveDir);
            await Promise.all(files.map((file) => fs.unlink(path.join(this.autoSaveDir, file))));
     ...
     * 保存自动保存文件
     */
    async saveAutoSaveFile(filePath, projectData) {
    ...
    async cleanupOldAutoSaves() {
        try {
            ...
            if (pantFiles.length > this.config.maxBackups) {
                const fileStats = await Promise.all(pantFiles.map(async (file) => {
                    const filePath = path.join(this.autoSaveDir, file);
                    const stats = await fs.stat(filePath);
    ...
    setupIpcHandlers() {
         ...
         * 设置当前项目
         */
        electron_1.ipcMain.on('autosave:setProject', (_event, projectPath, projectData) => {
            this.setCurrentProject(projectPath, projectData);
        });
        /**
         * 清除当前项目
         */
        electron_1.ipcMain.on('autosave:clearProject', () => {
            this.clearCurrentProject();
        });
        /**
         * 手动触发自动保存
         */
        electron_1.ipcMain.on('autosave:trigger', async () => {
            await this.performAutoSave();
        });
        /**
         * 获取自动保存列表
         */
        electron_1.ipcMain.handle('autosave:getList', async () => {
            return this.getAutoSaveList();
        });
        /**
         * 恢复自动保存
         */
        electron_1.ipcMain.handle('autosave:restore', async (_event, filePath) => {
            return this.restoreAutoSave(filePath);
        });
        /**
         * 删除自动保存文件
         */
        electron_1.ipcMain.handle('autosave:delete', async (_event, filePath) => {
            return this.deleteAutoSave(filePath);
        });
        /**
         * 清空所有自动保存
         */
        electron_1.ipcMain.handle('autosave:clearAll', async () => {
            return this.clearAllAutoSaves();
        });
        /**
         * 启动自动保存
         */
        electron_1.ipcMain.on('autosave:start', () => {
            this.startAutoSave();
        });
        /**
         * 停止自动保存
         */
        electron_1.ipcMain.on('autosave:stop', () => {
            this.stopAutoSave();
        });
        /**
         * 设置自动保存间隔
         */
        electron_1.ipcMain.on('autosave:setInterval', (_event, interval) => {
